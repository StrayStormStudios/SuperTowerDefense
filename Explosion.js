...
Explosion.prototype.calculateArrays = function () {
  for (var i = 0; i < this.growthFrames; i++) {
    this.radii.push(this.initialRadius + 1.0 * (this.fullSizeRadius - this.initialRadius) * (i / this.growthFrames));

    this.alphas.push(this.initialAlpha + 1.0 * (this.fullSizeAlpha - this.initialAlpha) * (i / this.growthFrames));
  }
  for (var i = 0; i <= this.fadeFrames; i++) {
    this.radii.push(this.fullSizeRadius);
    this.alphas.push(this.fullSizeAlpha + 1.0 * (this.finalAlpha - this.fullSizeAlpha) * (i / this.fadeFrames));
  }
};

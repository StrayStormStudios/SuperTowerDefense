...
*/
function Tower(position, rotation,
	range, damage, armorDamage, framesPerShot, bulletSpeed, numberShots,
	explodes, explosionRadius, explosionDamage, explosionArmorDamage,
	upgradesOne, upgradesTwo,
	buffRadius, buffDamage, buffArmorDamage, buffRange, buffFramesPerShot,
	drawType, color1, color2, color3, color4, highlightColor,
	bulletDrawingType, bulletRadius, bulletColor, explosionColor,
	explosionInitialRadius, explosionGrowthFrames, explosionInitialAlpha, explosionFullSizeAlpha,
	explosionFadeFrames, explosionFinalAlpha,
	name, cost, framesPerMoney){
	//id
	this.id = Tower.id;
	Tower.id++;
...
*/
Tower.prototype.draw = function(ctx, isButton, isClicked, isDragged, greenRadius){
	...
	if(this.drawType == 1){
		TowerDrawing.draw1(ctx, this.position.x, this.position.y, scale, scale, 
			this.rotation, this.isSelected || isClicked, isButton, 
			this.color1, this.color2, this.color3, this.color4, this.highlightColor);
	} else if (this.drawType==2){
		TowerDrawing.draw2(ctx, this.position.x, this.position.y, scale, scale, 
			this.rotation, this.isSelected || isClicked, isButton, 
			this.color1, this.color2, this.color3, this.color4, this.highlightColor);
	} else if (this.drawType==3){
		TowerDrawing.draw3(ctx, this.position.x, this.position.y, scale, scale, 
			this.rotation, this.isSelected || isClicked, isButton, 
			this.color1, this.color2, this.color3, this.color4, this.highlightColor);
	} else if (this.drawType==4){
		TowerDrawing.draw4(ctx, this.position.x, this.position.y, scale, scale, 
			this.rotation, this.isSelected || isClicked, isButton, 
			this.color1, this.color2, this.color3, this.color4, this.highlightColor);
	} else if (this.drawType==5){
		TowerDrawing.draw5(ctx, this.position.x, this.position.y, scale, scale, 
			this.rotation, this.isSelected || isClicked, isButton, 
			this.color1, this.color2, this.color3, this.color4, this.highlightColor);
	} else if (this.drawType == 6){
		TowerDrawing.draw6(ctx, this.position.x, this.position.y, scale, scale, 
			this.rotation, this.isSelected || isClicked, isButton, 
			this.color1, this.color2, this.color3, this.color4, this.highlightColor);
	}
...
Tower.prototype.increaseUpgrade = function(upgrade){
	...
	if(currentUpgrade != null && currentUpgrade.cost <= SuperTower.cash){
		...
		if(currentUpgrade.type=="armorDamage") this.armorDamage += currentUpgrade.value;
		...
		if(currentUpgrade.type=="bulletSpeed") this.bulletSpeed += currentUpgrade.value;
		if(currentUpgrade.type=="numberShots") this.numberShots += currentUpgrade.value;
...
Tower.prototype.targetEnemy = function(){
	...
	for(var i = 0, m = this.fireRadius.length; i < m; i++){
		...
		for(var j = 0, n = SuperTower.enemies.enemies.length; j < n; j++){
			...
			if(!curEnemy.isDead){
				if(bestFramesTillEnd == -1 || 
					curEnemy.framesTillEnd(frame) < bestFramesTillEnd){
						if(curEnemy.inRadius(this.position, this.fireRadius[i], frame + i + 1)){
...
Tower.prototype.fire = function(){
	if(this.shotCooldown > 0){
		...
		if(this.enemyTargeted != null){
			...
			//damage enemy
			this.enemyTargeted.applyDamage(this.frameTargeted, this.damage, this.armorDamage);
			//add a bullet
			SuperTower.bullets.add(this.position, this.rotation, this.enemyTargeted, this.frameTargeted, 
				this.bulletSpeed, this.bulletDrawingType, this.bulletRadius, this.bulletColor);
			//should we explode?
			if(this.explodes && this.explosionRadius > 0){
				//damage all enemies if appropriate
				//loop through all other enemies
				//draw the explosion
				var targetPosition = this.enemyTargeted.positions[this.frameTargeted - 
							this.enemyTargeted.frame];
				SuperTower.explosions.add(targetPosition, 
					this.explosionColor, this.explosionInitialRadius, 
					this.explosionRadius, this.explosionGrowthFrames,  
					this.explosionInitialAlpha, this.explosionFullSizeAlpha,
					this.explosionFadeFrames, this.explosionFinalAlpha, this.frameTargeted);
				...
				for(var j = 0, n = SuperTower.enemies.enemies.length; j < n; j++){
					...
					if(!curEnemy.isDead){
						...
						//apply the damage
						if(curEnemy.inRadius(targetPosition, this.explosionRadius, 
							this.frameTargeted)){
							curEnemy.applyDamage(this.frameTargeted, 
...
Tower.prototype.calculateBuffs = function(){
	...
	if(this.range > 0 && this.damage >= 0 && this.numberShots >= 0 && this.bulletSpeed >= 0){

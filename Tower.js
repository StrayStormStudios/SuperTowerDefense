...
Tower.prototype.increaseUpgrade = function (upgrade) {
  ...
  if (currentUpgrade != null && currentUpgrade.cost <= SuperTower.cash) {
    ...
    if (currentUpgrade.type == "explosionRadius") this.explosionRadius += currentUpgrade.value;
    if (currentUpgrade.type == "explosionDamage") this.explosionDamage += currentUpgrade.value;
    if (currentUpgrade.type == "explosionArmorDamage") this.explosionArmorDamage += currentUpgrade.value;
...
Tower.prototype.targetEnemy = function () {
  ...
  for (var i = 0, m = this.fireRadius.length; i < m; i++) {
    ...
    for (var j = 0, n = SuperTower.enemies.enemies.length; j < n; j++) {
      ...
      if (!curEnemy.isDead) {
        if (bestFramesTillEnd == -1 || curEnemy.framesTillEnd(frame) < bestFramesTillEnd) {
  ...
  if (this.shotCooldown > 0) {
    ...
    if (this.enemyTargeted != null) {
      ...
      if (this.explodes && this.explosionRadius > 0) {
        ...
        //draw the explosion
        var targetPosition = this.enemyTargeted.positions[this.frameTargeted - this.enemyTargeted.frame];
        ...
        for (var j = 0, n = SuperTower.enemies.enemies.length; j < n; j++) {
          ...
          if (!curEnemy.isDead) {
            ...
            //apply the damage
            if (curEnemy.inRadius(targetPosition, this.explosionRadius, this.frameTargeted)) {
              curEnemy.applyDamage(this.frameTargeted, this.explosionDamage, this.explosionArmorDamage);
            }
          }
        }
      }

...
Path.prototype.drawStartCap = function (ctx) {
  ...
  ctx.quadraticCurveTo(this.startPoints[3].x, this.startPoints[3].y, this.startPoints[0].x, this.startPoints[0].y);
  ctx.closePath();
  ctx.fill();
};
...
Path.prototype.drawEndCap = function (ctx) {
  ...
  ctx.quadraticCurveTo(this.endPoints[3].x, this.endPoints[3].y, this.endPoints[0].x, this.endPoints[0].y);
  ctx.closePath();
  ctx.fill();
};
...
Path.prototype.getRectangles = function () {
  for (var i = 0, n = this.points.length - 1; i < n; i++) {
    ...
    var p2Big = new Point(x2 + pdx * 1.1 + pVector.x * 1.3 * this.width, y2 + pdy * 1.1 + pVector.y * 1.3 * this.width);
    ...
    var p3Big = new Point(x3 - pdx * 1.1 + pVector.x * 1.3 * this.width, y3 - pdy * 1.1 + pVector.y * 1.3 * this.width);
    ...
    var p4Big = new Point(x4 - pdx * 1.1 - pVector.x * 1.3 * this.width, y4 - pdy * 1.1 - pVector.y * 1.3 * this.width);
...
Path.prototype.getEndPoints = function () {
  ...
  var p1 = new Point(
    this.points[this.rectangles.length].x + this.vectors[this.rectangles.length - 1].x * -this.width * 2,
    this.points[this.rectangles.length].y + this.vectors[this.rectangles.length - 1].y * -this.width * 2
  );
  var p2 = new Point(
  ...
  var p3 = new Point(
    this.points[this.rectangles.length].x + this.vectors[this.rectangles.length - 1].x * this.width,
    this.points[this.rectangles.length].y + this.vectors[this.rectangles.length - 1].y * this.width
  );
...
Path.prototype.inPath = function (p) {
  for (var i = 0, n = this.uvInverses.length; i < n; i++) {
    ...
    var pPrime = new Point(p.x - this.rectanglesBig[i][0].x, p.y - this.rectanglesBig[i][0].y);
    ...
    if (pTransformed.x >= 0 && pTransformed.x <= 1 && pTransformed.y >= 0 && pTransformed.y <= 1) {
      return true;
    }
  }
  return false;

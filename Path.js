...
Path.prototype.drawRectangles = function(ctx){
	...
	for(var i = 0; i < this.rectangles.length; i++){
		...
		var grd = ctx.createLinearGradient(this.rectangles[i][0].x, this.rectangles[i][0].y, 
			this.rectangles[i][3].x, this.rectangles[i][3].y);
		grd.addColorStop(0.000, this.color);
		grd.addColorStop(0.500, this.highlightColor);
		grd.addColorStop(1.000, this.color);
		ctx.fillStyle = grd;
		//draw geometry
		ctx.beginPath();
		ctx.moveTo(this.rectanglesPrint[i][0].x,
			this.rectanglesPrint[i][0].y);
		ctx.lineTo(this.rectanglesPrint[i][1].x,
			this.rectanglesPrint[i][1].y);
		ctx.lineTo(this.rectanglesPrint[i][2].x,
			this.rectanglesPrint[i][2].y);
		ctx.lineTo(this.rectanglesPrint[i][3].x,
			this.rectanglesPrint[i][3].y);
		ctx.lineTo(this.rectanglesPrint[i][0].x,
			this.rectanglesPrint[i][0].y);
		ctx.closePath();
		ctx.fill();
	}
...
Path.prototype.drawIntersections = function(ctx){	
	...
	for(var i = 0; i < this.intersections.length; i++){
		...
		grd.addColorStop(0.500, this.highlightColor);
		grd.addColorStop(1.000, this.color);
		var grd2 = ctx.createLinearGradient(this.rectangles[i+1][0].x, this.rectangles[i+1][0].y, 
			this.rectangles[i+1][3].x, this.rectangles[i+1][3].y);
		grd2.addColorStop(0.000, this.color);
		grd2.addColorStop(0.500, this.highlightColor);
		grd2.addColorStop(1.000, this.color);
		ctx.fillStyle = grd2;
		//geometry
		ctx.beginPath();
		ctx.moveTo(this.intersections[i][0].x,
			this.intersections[i][0].y);
		ctx.lineTo(this.intersections[i][1].x,
			this.intersections[i][1].y);
		ctx.quadraticCurveTo(this.intersections[i][2].x,
			this.intersections[i][2].y,
			this.intersections[i][3].x,
			this.intersections[i][3].y);
		ctx.lineTo(this.intersections[i][0].x,
			this.intersections[i][0].y);
		ctx.closePath();
		ctx.fill();
		...
		ctx.lineTo(this.intersections[i][1].x, 
			this.intersections[i][1].y);
		ctx.lineTo(this.intersections[i][2].x, 
			this.intersections[i][2].y);
		ctx.lineTo(this.intersections[i][0].x, 
			this.intersections[i][0].y);
		ctx.clip();
...
Path.prototype.drawStartCap = function(ctx){
	...
	var startGrd = ctx.createLinearGradient(this.rectangles[0][0].x, this.rectangles[0][0].y, 
		this.rectangles[0][3].x, this.rectangles[0][3].y);
	startGrd.addColorStop(0.000, this.startColor);
	startGrd.addColorStop(0.500, this.startHighlightColor);
	startGrd.addColorStop(1.000, this.startColor);
	ctx.fillStyle = startGrd;
	//geometry
	ctx.beginPath();
	ctx.moveTo(this.startPoints[0].x,
		this.startPoints[0].y);
	ctx.lineTo(this.startPoints[1].x,
		this.startPoints[1].y);
	ctx.lineTo(this.startPoints[2].x,
		this.startPoints[2].y);
	ctx.quadraticCurveTo(this.startPoints[3].x,
...
Path.prototype.drawEndCap = function(ctx){
	...
	var endGrd = ctx.createLinearGradient(this.rectangles[this.rectangles.length-1][0].x,
	...
	endGrd.addColorStop(0.000, this.endColor);
	endGrd.addColorStop(0.500, this.endHighlightColor);
	endGrd.addColorStop(1.000, this.endColor);
	ctx.fillStyle = endGrd;
	//geometry	
	ctx.beginPath();
	ctx.moveTo(this.endPoints[0].x,
		this.endPoints[0].y);
	ctx.lineTo(this.endPoints[1].x,
		this.endPoints[1].y);
	ctx.lineTo(this.endPoints[2].x,
		this.endPoints[2].y);
	ctx.quadraticCurveTo(this.endPoints[3].x,
		this.endPoints[3].y, 
		this.endPoints[0].x,
		this.endPoints[0].y);
	ctx.closePath();
...
Path.prototype.getRectangles = function(){
	for(var i = 0, n = this.points.length-1; i < n; i++){
		...
		var pVector;
		if(dMag == 0){ //zero length vectors... just push the original ones
		...
		var p1Big = new Point(p1.x + pdx * 1.1 - pVector.x * 1.3*this.width, 
		...
		var p2Big = new Point(x2 + pdx * 1.1 + pVector.x * 1.3*this.width,
		...
		var p3Big = new Point(x3  - pdx * 1.1 + pVector.x * 1.3*this.width,
		...
		var p4Big = new Point(x4 - pdx * 1.1 - pVector.x * 1.3*this.width,
...
Path.prototype.getIntersections = function(){
	...
	for(var i = 0; i < this.rectangles.length-1; i++){
		...
		var q1 = new Point(this.rectangles[i+1][1].x, this.rectangles[i+1][1].y);
		var q2 = new Point(this.rectangles[i+1][2].x, this.rectangles[i+1][2].y);
				
		//get r and s vectors (for top(1) and bottom(2))
		var r1 = new Point(this.rectangles[i][1].x - p1.x, this.rectangles[i][1].y - p1.y);
		var r2 = new Point(this.rectangles[i][2].x - p2.x, this.rectangles[i][2].y - p2.y);
		var s1 = new Point(this.rectangles[i+1][0].x - q1.x, this.rectangles[i+1][0].y - q1.y);
		var s2 = new Point(this.rectangles[i+1][3].x - q2.x, this.rectangles[i+1][3].y - q2.y);
...
Path.prototype.getStartPoints = function(){
	...
	var p1 = new Point(this.points[0].x + this.vectors[0].x * this.width * 2, 
		this.points[0].y + this.vectors[0].y * this.width* 2);
	var p2 = new Point(this.rectangles[0][3].x, this.rectangles[0][3].y);
	var p3 = new Point(this.points[0].x + -1 * this.vectors[0].x * this.width, 
...
Path.prototype.getEndPoints = function(){
	var p0 = new Point(this.rectangles[this.rectangles.length-1][1].x,
		this.rectangles[this.rectangles.length-1][1].y);
	var p1 = new Point(this.points[this.rectangles.length].x + 
	...
	var p2 = new Point(this.rectangles[this.rectangles.length-1][2].x,
		this.rectangles[this.rectangles.length-1][2].y);
	var p3 = new Point(this.points[this.rectangles.length].x + 
...
Path.prototype.getUVInverses = function(){
	...
	//for(var i = 1, n = 2; i < n; i++){ 
		...
		var u = new Point(this.rectanglesBig[i][1].x - this.rectanglesBig[i][0].x, 
			this.rectanglesBig[i][1].y - this.rectanglesBig[i][0].y);
		var v = new Point(this.rectanglesBig[i][3].x - this.rectanglesBig[i][0].x, 
...
Path.prototype.inPath = function(p){
	for(var i = 0, n = this.uvInverses.length; i < n; i++){
		...
		var pPrime = new Point(p.x - this.rectanglesBig[i][0].x, 
		...
		if(pTransformed.x >= 0 && pTransformed.x <= 1 &&
			pTransformed.y >= 0 && pTransformed.y <= 1){
				return true;
		} 
	}
	return false;

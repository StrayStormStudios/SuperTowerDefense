...
EnemyWaves.prototype.spawnEnemies = function(frame){
	...
	while(this.curIndex < this.waves.length && this.waves[this.curIndex].stage < SuperTower.stage){
		this.curIndex++;
	}
	...
	while(this.curIndex < this.waves.length && framesElapsed > 0){
		...
		if(this.waves[this.curIndex].delay == 0){
			SuperTower.enemies.add(this.waves[this.curIndex].type, 
				frame, 
	SuperTower.stageList.stages[this.waves[this.curIndex].stage].paths[this.waves[this.curIndex].path],
				0);

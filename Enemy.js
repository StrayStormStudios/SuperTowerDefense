...
*/
function Enemy(type, health, armor, money, speed, damage,
	color1, color2, color3, color4, drawingFunction, frame, path, deathSplit, distanceTravelled){
	//set id
	this.id = Enemy.id;
	Enemy.id++;
	...
	this.money = money;
	this.speed = speed + (Math.random() * .01);
	this.damage = damage;
	this.color1 = color1;
	this.color2 = color2;
	this.color3 = color3;
	...
	this.hitRadius = this.path.width * .9;
...
Enemy.prototype.calculatePositions = function(path){ 
	...
	while(this.distanceTravelled <= this.path.totalLength){
		...
		//see if we need a new path?
			...
			var x = this.path.points[pathSegment].x + 
				this.path.vectors[pathSegment].x * currentPathTravelled;
			var y = this.path.points[pathSegment].y + 
...
Enemy.prototype.draw = function(ctx, currentFrame){ 
	...
	if(currentIndex < this.positions.length){
		...
		if(this.drawingFunction == 1){
			EnemyDrawing.draw1(ctx, x, y, rotation, 
				pathWidth, this.color1, this.color2, this.color3, this.color4);	
		} else if(this.drawingFunction == 2){
			EnemyDrawing.draw2(ctx, x, y, rotation, pathWidth, 
				this.color1, this.color2, this.color3, this.color4);
		} else if(this.drawingFunction == 3){
			EnemyDrawing.draw3(ctx, x, y, rotation, pathWidth, 
				this.color1, this.color2, this.color3, this.color4);
		} else if(this.drawingFunction == 4){
			EnemyDrawing.draw4(ctx, x, y, rotation, pathWidth, 
				this.color1, this.color2, this.color3, this.color4);
		} else if(this.drawingFunction == 5){
			EnemyDrawing.draw5(ctx, x, y, rotation, pathWidth, 
				this.color1, this.color2, this.color3, this.color4);
		} else if(this.drawingFunction == 6){
			EnemyDrawing.draw6(ctx, x, y, rotation, pathWidth, 
				this.color1, this.color2, this.color3, this.color4);
		} else if(this.drawingFunction == 7){
			EnemyDrawing.draw7(ctx, x, y, rotation, pathWidth, 
				this.color1, this.color2, this.color3, this.color4);
		} else if(this.drawingFunction == 8){
			EnemyDrawing.draw8(ctx, x, y, rotation, pathWidth, 
				this.color1, this.color2, this.color3, this.color4);
		} else if(this.drawingFunction == 9){
			EnemyDrawing.draw9(ctx, x, y, rotation, pathWidth, 
				this.color1, this.color2, this.color3, this.color4);
		} else if(this.drawingFunction == 10){
			EnemyDrawing.draw10(ctx, x, y, rotation, pathWidth, 
				this.color1, this.color2, this.color3, this.color4);
		} 

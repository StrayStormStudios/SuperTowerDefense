...
function TowerMenu(){
	...
	for(var i = 1; i <= this.topLeft.length; i++){
		this.towers.push(TowerType.createTower(new Point(this.topLeft[i-1].x+29,this.topLeft[i-1].y+29),
			0, i));
	}
...
TowerMenu.prototype.getHighlighted = function(){
	...
	if(this.towerSelected != -1){
		...
		for(var i = 0, n = this.topLeft.length; i < n; i++){
			if(mouseX >= this.topLeft[i].x && mouseX <= this.topLeft[i].x + this.width  &&
				mouseY >= this.topLeft[i].y && mouseY <= this.topLeft[i].y + this.height){
		...
		for(var i = 0, n = this.upgradeTopLeft.length; i < n; i++){
			if(mouseX >= this.upgradeTopLeft[i].x && 
		...
		if(mouseX >= this.topLeft[i].x && mouseX <= this.topLeft[i].x + this.width  &&
			mouseY >= this.topLeft[i].y && mouseY <= this.topLeft[i].y + this.height){
				...
				if(this.towers[i].cost <= SuperTower.cash){
					...
					this.draggedTower = TowerType.createTower(new Point(mouseX, mouseY),0,i+1);
...
TowerMenu.prototype.mouseUp = function(){
	if(this.draggingTower != false){
		...
		if(this.validPosition()){
			SuperTower.towers.add(new Point(SuperTower.mouseX,SuperTower.mouseY), 0, this.towerSelected);
...
TowerMenu.prototype.validPosition = function(){
		//off screen
		if(this.draggedTower.position.x <= Background.SCREEN_X ||
			this.draggedTower.position.x >= Background.SCREEN_X + Background.SCREEN_WIDTH ||
			this.draggedTower.position.y <= Background.SCREEN_Y ||
			this.draggedTower.position.y >= Background.SCREEN_Y + Background.SCREEN_HEIGHT){
			return false;
		}
		
		//what is the closest distance to another tower?
		var closestDistance = SuperTower.stageList.stages[SuperTower.stage].paths[0].width * 2.2;
		...
		for(var i = SuperTower.towers.towers.length - 1; i >= 0; i--){
			var xDiff = SuperTower.towers.towers[i].position.x - this.draggedTower.position.x;
			var yDiff = SuperTower.towers.towers[i].position.y - this.draggedTower.position.y;
		...
		//too close to a path
		for(var i = 0, n = SuperTower.stageList.stages[SuperTower.stage].paths.length; i < n; i++){
			if(SuperTower.stageList.stages[SuperTower.stage].paths[i].inPath(this.draggedTower.position)){
				return false;
			}
		}
		return true;
